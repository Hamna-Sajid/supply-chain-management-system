...
export const acceptShipment = async (req, res) => {
  try {
    ...
    res.status(error.message === 'Shipment not found' ? 404 : 500).json({ error: error.message || 'Server error' });
  }
};

export const rejectShipment = async (req, res) => {
  try {
    const result = await warehouseService.rejectShipment(req.params.id, req.user.userId);
    res.json(result);
  } catch (error) {
...
export const updateShipmentStatus = async (req, res) => {
  ...
  try {
    const result = await warehouseService.updateShipmentStatus(req.params.id, req.user.userId, status);
    res.json(result);
  } catch (error) {
...
export const updateOrderStatus = async (req, res) => {
  ...
  try {
    const result = await warehouseService.updateOrderStatus(req.params.id, req.user.userId, status);
    res.json(result);
  } catch (error) {
    console.error('updateOrderStatus error:', error);
    res.status(error.message.includes('not found') ? 404 : 500).json({ error: error.message || 'Server error' });
  }
};