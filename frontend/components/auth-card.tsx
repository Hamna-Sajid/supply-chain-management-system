...
export function AuthCard() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Use the global auth context login to sync app state
      await login({ email: loginEmail, password: loginPassword });
      
      // Get the stored user to check their role for routing
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Create the account via the API
      ...
      // 2. Auto-login using the global auth context to sync state
      await login({ email: signupEmail, password: signupPassword });
      
      // 3. Route based on the role they just registered with
      if (signupRole === 'warehouse_manager') {
